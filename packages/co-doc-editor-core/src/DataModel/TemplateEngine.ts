import {dataPathLength, MultiDataPath, parsePath} from './DataPath';
...
export class TemplateLine {
  ...
  private static parse(line: string): TemplateToken[] {
    ...
    for (;;) {
      ...
      let path: MultiDataPath | undefined;
  ...
  public fill(data: DataModel | undefined, index: string | undefined): string {
    let startIndex = 0;
    let line = '';
    for (const token of this.tokens) {
      ...
      if (token.path) {
        if (dataPathLength(token.path) === 0 && token.path.pointsKey) {
          line += index;
        } else if (data) {
          const resolvedValue = (token.path, data);
          if (resolvedValue) {
